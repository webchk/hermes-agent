...
export function getActiveMemoryProvider(profile?: string): string {
  try {
    ...
    if (!existsSync(configPath)) return "holographic";
    const content = readFileSync(configPath, "utf-8");
    const match = content.match(/^\s*provider:\s*["']?(\w+)["']?\s*$/m);
    return match?.[1] || "holographic";
  } catch {
    return "holographic";
  }
}
...
function parseMcpServersFromConfig(
  content: string,
  ...
  while ((m = nameRe.exec(block)) !== null) {
    ...
    servers.push({ name, type: hasUrl ? "http" : "stdio", enabled, detail });
  }
  return servers;
}
...
export function listMcpServers(
  ...
  // Profile-specific config takes priority
  try {
    const profileConfig = join(profileHome(profile), "config.yaml");
    if (existsSync(profileConfig)) {
      const content = readFileSync(profileConfig, "utf-8");
      for (const s of parseMcpServersFromConfig(content)) {
        seen.add(s.name);
        servers.push(s);
      }
    }
  ...
  // Global config — always merged so every workspace sees all MCPs
  try {
    const globalConfig = HERMES_CONFIG_FILE;

...
import {
  registerHermesMediaProtocol,
  HERMES_MEDIA_PRIVILEGES,
  readMediaAsDataUri,
} from "./media-protocol";
import { listN8nWorkflows, triggerN8nWorkflow } from "./n8n";
import {
...
} from "./agent-browser-bootstrap";

// Souza/JS local fix: register `hermes-media://` as a privileged scheme so it
// can be served by the main process and rendered inline by the renderer
// (see media-protocol.ts and AgentMarkdown.tsx). Must run BEFORE app.whenReady.
protocol.registerSchemesAsPrivileged([HERMES_MEDIA_PRIVILEGES]);
...
function setupIPC(): void {
  // Souza/JS: fallback for the hermes-media:// protocol — reads a whitelisted
  // file from disk and returns it as a `data:` URI. The renderer falls back
  // to this when the custom protocol fails for any reason (CSP, Electron
  // version regression, etc.). See media-protocol.ts::readMediaAsDataUri.
  ipcMain.handle(
    "read-media-as-data-uri",
    (_event, path: string) => readMediaAsDataUri(path),
  );

  // Souza/JS: n8n bridge — list/trigger workflows from the local n8n
  // instance (configured via N8N_URL / N8N_API_KEY in ~/.hermes/.env)
  ipcMain.handle("list-n8n-workflows", () => listN8nWorkflows());
  ipcMain.handle("trigger-n8n-workflow", (_event, id: string | number) =>
    triggerN8nWorkflow(id),
  );
  ...
  ipcMain.handle("get-config", (_event, key: string, profile?: string) => {
    const conn = getConnectionConfig();
    const value = conn.mode === "ssh" && conn.ssh
      ? sshGetConfigValue(conn.ssh, key, profile)
      : getConfigValue(key, profile);
    if (key === "memory.provider" && (value === null || value === "")) {
      return "holographic";
    }
    return value;
  });
    ...
    async (
      ...
      const handle = await sendMessage(
        message,
        {
          ...
          // Souza/JS: structured tool lifecycle events for ActivityFeed
          onToolEvent: (event) => {
            safeSend("chat-tool-event", event);
          },
          onUsage: (usage) => {
  ...
  ipcMain.handle("deepsproxy-install", async () => {
    await installDeepsProxy((chunk) => {
      mainWindow?.webContents.send("deepsproxy-install-log", chunk);
    });
  });

  ipcMain.handle("deepsproxy-start", async () => {
...
app.whenReady().then(() => {
  ...
  // Souza/JS: register custom protocol handler for inline media rendering
  registerHermesMediaProtocol();

  // Souza/JS: bootstrap vercel-labs/agent-browser — copia o binário Rust
  // empacotado para ~/.hermes/bin/agent-browser (idempotente; só copia se
  // version-hash diferente). Plugin Python `agent-browser` lê o path daí.
  bootstrapAgentBrowser();
  registerAgentBrowserIpc();

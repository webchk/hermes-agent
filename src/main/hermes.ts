...
import {
  processCustomEvent as sseProcessCustomEvent,
  type ToolEvent,
} from "./sse-parser";
...
export interface ChatCallbacks {
  ...
  /** Souza/JS: structured tool lifecycle events for ActivityFeed UI. */
...
function sendMessageViaApi(
   ...
   *
   * Souza/JS: delegates to sse-parser.processCustomEvent so both legacy
   * onToolProgress (string) and structured onToolEvent callbacks fire.
   * Structured events are forwarded to the renderer via IPC for the
   * ActivityFeed component (richer than the single-string toolProgress
   * state).
   */
  function processCustomEvent(eventType: string, data: string): void {
    sseProcessCustomEvent(eventType, data, {

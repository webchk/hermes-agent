...
async function findAvailablePortInRange(start: number, end: number): Promise<number> {
  for (let port = start; port <= end; port++) {
    const inUse = await checkPort(port);
    if (!inUse) return port;
  }
  return start; // fallback: retorna o início se todas estiverem ocupadas
}
...
export async function startAll(): Promise<{ success: boolean; error?: string }> {
  ...
  // Encontra porta disponível no range 4001-4100 e persiste
  const port = await findAvailablePortInRange(PORT_RANGE_START, PORT_RANGE_END);
  setClaw3dPort(port);

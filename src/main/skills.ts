...
/**
 * Walk a single skills directory and collect all installed skills.
  ...
  try {
    ...
    for (const category of categories) {
      ...
      for (const entry of entries) {
        ...
        if (seen.has(key)) continue; // profile-specific takes priority
        seen.add(key);
        ...
        try {
          ...
          skills.push({ name: entry, category, description: "", path: entryPath });
        }
      }
    }
  } catch {
...
/**
 * Walk the skills directories to find all installed skills.
 * Always merges global skills (~/.hermes/skills) with profile-specific skills
 * so every workspace has access to the full capability set.
 * Profile-specific skills take priority over global ones with the same key.
 */
export function listInstalledSkills(profile?: string): InstalledSkill[] {
  const seen = new Set<string>();
  const skills: InstalledSkill[] = [];

  // Profile-specific skills first (take priority)
  const profileDir = join(profileHome(profile), "skills");
  skills.push(...collectSkillsFromDir(profileDir, seen));

  // Global skills — always included regardless of profile
  const globalDir = join(HERMES_HOME, "skills");
  if (globalDir !== profileDir) {
    skills.push(...collectSkillsFromDir(globalDir, seen));
  }

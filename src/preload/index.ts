...
const hermesAPI = {
  ...
  // Souza/JS: structured tool lifecycle events for ActivityFeed component.
  // Receives ToolEvent {toolCallId, tool, status, label, emoji, args, result, durationMs, receivedAt}
  onChatToolEvent: (callback: (event: unknown) => void): (() => void) => {
  ...
  // Souza/JS: fallback loader for inline images when hermes-media:// fails
  readMediaAsDataUri: (
    path: string,
  ): Promise<{
    uri?: string;
    ...
    mime?: string;
    size?: number;
  }> => ipcRenderer.invoke("read-media-as-data-uri", path),
  ...
  listN8nWorkflows: (): Promise<{
    ...
    url?: string;
    error?: string;
  }> => ipcRenderer.invoke("list-n8n-workflows"),
  triggerN8nWorkflow: (
    id: string | number,
  ): Promise<{ ok: boolean; error?: string; executionId?: string }> =>
    ipcRenderer.invoke("trigger-n8n-workflow", id),
  ...
  }> => ipcRenderer.invoke("deepsproxy-status"),

  deepsProxyInstall: (): Promise<void> =>
    ipcRenderer.invoke("deepsproxy-install"),

  deepsProxyStart: (): Promise<void> =>
    ipcRenderer.invoke("deepsproxy-start"),

  deepsProxyStop: (): Promise<void> =>
    ipcRenderer.invoke("deepsproxy-stop"),

  deepsProxyLogin: (): Promise<void> =>
    ipcRenderer.invoke("deepsproxy-login"),

  onDeepsProxyInstallLog: (cb: (log: string) => void): (() => void) => {
    const h = (_ev: Electron.IpcRendererEvent, log: string): void => cb(log);
    ipcRenderer.on("deepsproxy-install-log", h);
    return () => ipcRenderer.off("deepsproxy-install-log", h);
  },

  onDeepsProxyLog: (cb: (log: string) => void): (() => void) => {
    const h = (_ev: Electron.IpcRendererEvent, log: string): void => cb(log);
    ipcRenderer.on("deepsproxy-log", h);
    return () => ipcRenderer.off("deepsproxy-log", h);
  },

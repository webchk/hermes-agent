...
interface HermesAPI {
  ...
  /** Souza/JS: load a whitelisted file as data: URI (fallback when hermes-media:// fails). */
  readMediaAsDataUri: (path: string) => Promise<{
    uri?: string;
    error?: string;
    mime?: string;
    size?: number;
  }>;
  /** Souza/JS: list n8n workflows (requires N8N_API_KEY in ~/.hermes/.env). */
  ...
  /** Souza/JS: activate (trigger) an n8n workflow by id. */
  triggerN8nWorkflow: (
    id: string | number,

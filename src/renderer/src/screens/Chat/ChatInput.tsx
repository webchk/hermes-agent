...
interface ChatInputProps {
  isLoading: boolean;
  /**
   * Souza/JS: when true, the textarea & Send button stay enabled even while
   * `isLoading` is true. The parent uses this to support a local message
   * queue (user types while agent is busy; messages auto-send when free).
   * The Abort button still keys off `isLoading` so the user can stop the
   * current turn.
   */
  inputAlwaysEnabled?: boolean;
  hasSession: boolean;
...
export const ChatInput = forwardRef<ChatInputHandle, ChatInputProps>(
  function ChatInput(
    ...
    // Effective gate for the textarea / Send button. The Abort button keeps
    // its own logic (still keys off raw `isLoading`).
    const blocksInput = isLoading && !inputAlwaysEnabled;
    const { t } = useI18n();
    ...
    function handleSend(): void {
      ...
      if (!hasPayload || blocksInput) return;
      setSlashMenuOpen(false);
    ...
    const canSend =
      (input.trim().length > 0 || attachments.length > 0) && !blocksInput;
      ...
      <>
        ...
        <div className="chat-input-wrapper">
          ...
          <textarea
            ...
            autoFocus
          />
          {/* Souza/JS: quando inputAlwaysEnabled+isLoading, mostramos AMBOS
              os botões — Send (enfileira) E Stop (aborta o turn atual). Sem
              inputAlwaysEnabled, é o comportamento clássico: Stop substitui
              Send durante streaming. */}
          {isLoading && !inputAlwaysEnabled ? (
            <button
            ...
            <>
              ...
              <button
                ...
                title={
                  isLoading
                    ? "Enfileirar (agente terminará a anterior antes)"
                    : t("chat.send")
                }
              >
                <Send size={16} />
